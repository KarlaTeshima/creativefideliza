...
const AdminHeader: React.FC<AdminHeaderProps> = ({
  ...
  return <header className="py-4 px-4 bg-white shadow">
      <div className="container max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 font-medium">Sistema de Fidelidade: CREATIVE PRINT</p>
        </div>
        <div className={`flex ${isMobile ? 'flex-col w-full' : 'flex-row'} gap-2`}>
          <Button variant="outline" asChild className={isMobile ? "w-full" : ""}>
            <Link to="/">Início</Link>
          </Button>
          <Button variant="destructive" onClick={logout} className={isMobile ? "w-full" : ""}>
            Sair
          </Button>
        </div>
      </div>
